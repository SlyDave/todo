...
if (command === 'list') {
  for (const label of labels)
    console.log(`${label.name.padEnd(24)} #${label.color}  ${label.description}`)
  console.log(
...
if (failures > 0) {
  fail(
    `${failures} label(s) could not be applied. Check \`gh auth status\` and that a GitHub remote exists.`
  )
  process.exit(1)
}

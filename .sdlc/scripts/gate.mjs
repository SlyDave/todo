...
if (!state || state.status === 'uninitialised') {
  fail('This project has not been bootstrapped, so there is nothing to check. Run the `bootstrap` skill.');
  process.exit(1);
}
...
const configured = ORDER.filter((name) => typeof checks[name] === 'string' && checks[name].length > 0);

if (configured.length === 0) {
  warn('No checks are configured in .sdlc/state.json. Bootstrap should have set these; the gate is toothless.');
}
...
console.log('');
if (failures > 0) {
  console.error(`${colours.red}Gate failed with ${failures} problem(s). Do not open a pull request.${colours.reset}`);
  process.exit(1);
}

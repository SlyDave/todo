...
try {
  ...
  cpSync(repoRoot, sandbox, {
    recursive: true,
    filter: (source) => !source.includes(`${join(repoRoot, '.git')}`) && !source.includes('.worktrees'),
  });
  ...
  writeFileSync(join(sandbox, '.sdlc', 'run', 'session.json'), JSON.stringify({ rootConversationId: 'ROOT' }));

  const hook = (payload) => {
    const result = run('node', ['.cursor/hooks/guard-writes.mjs'], { cwd: sandbox, input: JSON.stringify(payload) });
  ...
  const created = run('node', ['.sdlc/scripts/worktree.mjs', 'create', 'backend', '42'], { cwd: sandbox });
  ...
  const duplicate = run('node', ['.sdlc/scripts/worktree.mjs', 'create', 'backend', '43'], { cwd: sandbox });
  ...
  const refusedDirty = !dirtyGate.ok && (dirtyGate.stdout + dirtyGate.stderr).includes('uncommitted changes');
  check('gate refuses to run against an uncommitted tree', refusedDirty, dirtyGate.stdout + dirtyGate.stderr);
  ...
  check('gate rejects a branch that touched another role\u2019s paths', caught, badGate.stdout + badGate.stderr);

  // --- Cleanup ----------------------------------------------------------------

  const dirtyRemove = run('node', ['.sdlc/scripts/worktree.mjs', 'remove', 'backend'], { cwd: sandbox });
  check('worktree remove succeeds once the lane is committed', dirtyRemove.ok, dirtyRemove.stderr || dirtyRemove.stdout);

  // The branch here is unmerged, so it must survive; removal must not discard
  // work just because the worktree went away.
  const survived = run('git', ['rev-parse', '--verify', '--quiet', 'backend/issue-42'], { cwd: sandbox });
  check('worktree remove keeps an unmerged branch', survived.ok, 'the branch was deleted with work still on it');
  ...
  const merged = run('git', ['rev-parse', '--verify', '--quiet', 'backend/issue-44'], { cwd: sandbox });
  check('worktree remove deletes a fully merged branch', !merged.ok, 'the merged branch was left behind');

  // --- Derived distributions ---------------------------------------------------

  const distributions = run('node', ['.sdlc/scripts/sync-distributions.mjs', '--check'], { cwd: sandbox });
  ...
  check('label definitions parse', labels.ok && labels.stdout.includes('state:ready'), labels.stderr);
} finally {
  try {
...
console.log('');
if (failed.length > 0) {
  console.error(`${colours.red}${failed.length} of ${results.length} checks failed.${colours.reset}`);
  process.exit(1);
}

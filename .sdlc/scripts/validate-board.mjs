...
if (!ghSupportsIssueRelationships()) {
  warn(
    'gh 2.94 or later is required to read parent and blocked-by data. Some checks will be skipped.'
  )
}
...
for (const issue of issues) {
  ...
  if (states.length > 1)
    note(issue, `has ${states.length} state labels (${states.join(', ')}); exactly one is allowed`)
  ...
  if (states.includes('state:ready')) {
    ...
    if (blockers.length > 0) {
      note(
        issue,
        `is ready but is blocked by ${blockers.map((blocker) => `#${blocker.number}`).join(', ')}`
      )
    }
  ...
  if (states.includes('state:done')) {
    note(
      issue,
      'is marked done but is still open; it should have been closed by its merged pull request'
    )
  }
}
...
if (branches.ok) {
  ...
  for (const [issueNumber, held] of claims) {
    ...
    if (developerBranches.length > 1) {
      problems.push(
        `#${issueNumber} is claimed by more than one developer branch: ${developerBranches.join(', ')}`
      )
    }
  }
}
...
const closed = gh([
  ...
  '50',
  '--json',
  'number,title,labels,stateReason'
])
if (closed.ok) {
  let recentlyClosed = []
  try {
  ...
  for (const issue of recentlyClosed) {
    ...
    const stale = namesOf(issue).filter(
      (name) => name.startsWith('state:') && name !== 'state:done'
    )
    if (stale.length > 0) {

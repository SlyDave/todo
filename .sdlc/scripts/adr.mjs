...
if (command === 'proposed') {
  ...
  for (const record of proposed) {
    console.error(
      `  - ${record.title} (${record.file}${record.issue ? `, issue #${record.issue}` : ''})`
    )
  }
...
if (records.length === 0) {
  ...
  for (const status of STATUS_ORDER) {
    ...
    for (const record of group) {
      const detail = [record.date, record.issue ? `issue #${record.issue}` : null]
        .filter(Boolean)
        .join(', ')
  ...
  if (unknown.length > 0) {
    ...
    for (const record of unknown)
      lines.push(`- [${record.title}](${record.file}) - status: ${record.status}`)
    lines.push('')
  }
}
